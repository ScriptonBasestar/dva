...
func TestArchiveFrontmatter_reportsAnUnreadableCard(t *testing.T) {
	...
	if n != 4 {
		t.Errorf("%d error(s) name the read failure on %s, want 4 — the link scan, the archive guard, the duplicate-id guard, and the filename-number guard each read the file and each report it; if one stopped, this gate has less coverage than it reads as having", n, broken)
	}
	if res.OK {
