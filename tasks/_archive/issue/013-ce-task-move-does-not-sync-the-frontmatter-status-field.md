...
resolution-summary: "ce-agent-kit 8f2ed451 stamps frontmatter status in move, resolve, and archive; no-body-status tests pass and no live done card remains status: todo."
---

## Summary

Resolved in ce-agent-kit master `8f2ed451`; the observations below record the
original move/resolve/archive behavior.
...
- [x] `ce task move`가 카드를 이동시킬 때 frontmatter `status:`를 도착 존이
      허용하는 값으로 갱신한다 | verify: human — upstream 테스트가 `**Status**`
      테이블 셀이 없는 카드에 대해서도 frontmatter 갱신을 확인한다
- [x] DVA 저장소의 live 카드 중 `status: todo`인 채로 `tasks/done/`에 있는 카드가
...
- [x] 이 카드가 닫히는 시점의 `ce task move` 실행 결과(또는 upstream 픽스 링크)가
...
## Resolution summary (2026-09-25)

`ce-agent-kit@8f2ed4514e010ec61cae35ec18d6fa8ea4ea85b8`에서 `move`, `resolve`,
`archive` 모두 frontmatter status를 기록한다. 공통 writer와 body Status 셀이
없는 각 경로의 테스트가 통과했으며, 현재 `tasks/done/`에는 `status: todo` 카드가
없다. TASK-426의 독립 done-review와 전체 commit CI
`6b88756eacac6c7e04f6ec57bdab50d2`가 PASS다.
