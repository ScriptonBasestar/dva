...
resolution-summary: "ce-agent-kit 5d70c9d8 counts Resolution Criteria for issue preflight; observed ISSUE-020=2 and ISSUE-004=3."
---

## Summary

Resolved in ce-agent-kit master `5d70c9d8`; the reproduction below preserves the
original failure.
...
- [x] preflight가 issue 서식 카드의 기준을 센다 | verify: `! ce task preflight --zone done --json 2>/dev/null | /usr/bin/grep -q 'no-criteria'`
- [x] 그때까지 두 카드는 서식 그대로 둔다 — 기준을 두 벌로 만들지 않는다 | verify: `! /usr/bin/grep -rq --include='002-progress-calculation-conflict-between-ce-task-validate-and-planprogress.md' --include='003-task-362-historic-panic-is-no-longer-reproducible.md' '^## Completion Criteria$' tasks`
...
ce-agent-kit `5d70c9d8`에서 preflight가 issue의 `Resolution Criteria`를 세도록
수정됐다. 설치된 `ce task preflight --zone issue --json` 출력은 ISSUE-020 기준
2개, ISSUE-004 기준 3개이며 기준 없는 ISSUE-032·039는 0을 유지한다. 전체 commit
CI `70534773fe86bae9bc7227252b4f77cb`, TASK-428 독립 done-review가 통과했다.
두 기존 카드의 Resolution Criteria 서식은 그대로 유지됐다.
