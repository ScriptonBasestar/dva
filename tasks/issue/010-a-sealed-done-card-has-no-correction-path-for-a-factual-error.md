...
그 위에서, 낮다 — 오늘 알려진 정정 불가 인스턴스는 TASK-376과, 개명이 남긴
`tasks/done/386-…md`의 `TASK-383` 참조 둘(→ [[ISSUE-012]])이고, 틀린 문장은 완료
판정이 아니라 리뷰 요약의 범위 서술이다. 다만 **보드가 커질수록 조용히 늘어나는 종류의 부채**다. 봉인된
