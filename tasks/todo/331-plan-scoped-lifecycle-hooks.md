...
- [ ] plan 필터가 붙은 훅이 다른 plan에서 실행되지 않는다 (바인딩 잠정 — 설계 확정 후 groom에서 테스트 함수명으로 교체) | verify: `make test`
- [ ] 필터 없는 기존 훅은 모든 plan에서 그대로 실행된다(호환) (바인딩 잠정 — 설계 확정 후 groom에서 테스트 함수명으로 교체) | verify: `make test`
- [ ] `dva validate`가 존재하지 않는 plan 이름을 훅 필터에 쓰면 거부한다 (바인딩 잠정 — 설계 확정 후 groom에서 테스트 함수명으로 교체) | verify: `make test`
