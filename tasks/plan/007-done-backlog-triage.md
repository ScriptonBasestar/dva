...
### 2026-09-14 재측정 — 숫자와 배분이 둘 다 낡았다

위 2026-09-10 항목의 46장/9장은 더 이상 재고가 아니다. **2026-09-14, master `8b56802`
기준 `tasks/done/`은 70장**이고
그중 `quality-review` 키가 없는 카드가 **39장**이다. 소유 plan별 배분:
...
측정 명령(`tasks/done/*.md`의 첫 프론트매터 블록만 읽는다 — 본문의 인용을 세지 않기
위해 두 번째 `---`에서 멈춘다):
...
표가 기계가 아니라 사람의 재측정으로만 유지되는 이유다.
