...
> **낡음 (2026-09-14).** 아래 46장/9장은 이 시점의 관측값이다. 현재 수치는
> `ce task lint`가 낸다 — 아래 2026-09-14 절을 보라.
...
`UNFINALIZED DONE (N card(s) in done/ with no quality-review verdict)` 한 줄과 해당
카드 목록을 낸다. `ce task lint`는 `ce task gate`의 세 단계 중 하나이므로(validate +
lint + preflight) 이 수는 **통합 게이트가 돌 때마다 자동으로 재측정된다.**

아래 표는 그 대체물이 아니라 `8b56802`의 **스냅샷**이고, 지우지 않는 이유는 하나다 —
표의 세로축(어느 plan이 몇 장을 갖는가)은 `ce task lint`가 내지 않는 정보이고 그것이
이 계획이 실제로 소유한 판단이다. 가로의 수는 이미 낡았다: 같은 날 master `6fd352a`에서
done은 74장, 미리뷰는 37장이다. 손으로 유지되는 수는 적는 순간부터 낡는다는 것을
이 표가 하루 만에 증명했고, 그래서 수는 위임한다.

`8b56802` 기준 `tasks/done/`은 70장이고 그중 `quality-review` 키가 없는 카드가
**39장**이었다. 소유 plan별 배분:
...
스냅샷을 낸 측정 명령(`tasks/done/*.md`의 첫 프론트매터 블록만 읽는다 — 본문의
인용을 세지 않기 위해 두 번째 `---`에서 멈춘다). 재현용으로 남기며, 이 표를 갱신하려고
다시 돌리지 않는다 — 그 일은 `ce task lint`가 한다:

```
$ for f in tasks/done/*.md; do
    awk 'BEGIN{n=0}/^---[[:space:]]*$/{n++;if(n==2)exit;next} n==1&&/^quality-review:/{found=1} END{exit !found}' "$f" \
      || basename "$f" | cut -d- -f1
  done | wc -l
39
```
...
`children:`에서만 읽힌다. 카드 한 장만 보고는 자기 소유자를 알 수 없다는 뜻이고, 위
표의 세로축이 기계로 유지될 수 없는 이유다 — `ce task lint`는 카드를 세지만 누가
소유하는지는 묻지 않는다. 수를 게이트에 넘기고 배분만 남기는 경계가 여기서 나온다.

**PLAN-008은 진척 100%로 닫혔는데 자식 8장 전부가 리뷰 없이 done에 있다.** 이 계획이
...
**소유 plan이 없는 9장은 어느 계획의 종료 조건에도 들어 있지 않다.** 2026-09-10에
"독립 6장은 TASK-373으로 분리한다"고 한 그 자리가 다시 비었고 이번에는 9장이다. 바로
위 문단이 적은 교훈이 같은 문서 안에서 한 번 더 반복됐다.

#### 처리 방침 — 소급 리뷰하지 않는다

39장을 지금 사후 리뷰해도 provenance가 진짜가 아니다. 변경 시점의 맥락 없이 판정을
쓰는 것은 `waived`를 `pass`로 위장하는 것과 같고, 아래 §`waived` 절이 상류에 추가됐다
48장에 대해 기록한 함정을 39장 규모로 되풀이하는 일이다.

- 활성 plan(006·009)의 **22장**은 각 부모가 닫는 시점에 리뷰된다. §Order 5가 Tier C에
  대해 이미 정한 방식 그대로다.
- PLAN-008의 8장과 소유자 없는 9장, 합 **17장은 레거시로 선언한다.** 리뷰되지 않았다는
  사실이 이 절과 PLAN-008에 남는다. 소급 리뷰하지 않는다.
- 유입은 게이트가 막는다. `blocks:`를 선언한 done 카드는 `ce task validate`가 receipt를
  요구하고, [[TASK-388]]이 그 receipt를 둘 durable 경로를 열었으므로 그 걸림은 이제
  실제로 해소 가능하다. **부채의 유입은 게이트가 막고, 재고는 선언으로 닫는다.**

#### 2026-09-14 후속 — 유입 차단은 절반만 참이었다

위 방침의 마지막 줄("부채의 유입은 게이트가 막고, 재고는 선언으로 닫는다")은 같은 날
반증됐다. [[TASK-381]]이 이 선언 **이후** `done/`에 들어왔고, 리뷰 판정 없이 들어왔다.
`ce task lint`의 미리뷰 수가 하나 늘었고, 그 한 장은 위 배분의 세 바구니 어디에도 없다 —
활성 plan의 자식이 아니고, 레거시로 선언한 17장에도 들어 있지 않다.

원인은 [[ISSUE-015]]에 실측과 함께 기록했다. 요약하면 `ce task validate`가 영수증을
요구하는 조건은 **`blocks:`로 후속 카드를 열어 줬는가** 하나뿐이고, 후속을 열지 않는
카드는 요구받지 않는다. TASK-381은 `blocks:`가 없다. 검사 자체는 정상 동작했다 —
막으려던 것("기록되지 않은 리뷰를 근거로 다른 카드를 열어 준 것")을 정확히 막았을 뿐,
그것이 유입 차단 일반이 아니었다.

**TASK-381의 배분: 소급 리뷰가 아니라 실제 독립 리뷰다.** 레거시 17장을 소급 리뷰하지
않기로 한 근거는 provenance였다 — 2026-09-10의 변경 맥락 없이 오늘 판정을 쓰면
`waived`를 `pass`로 위장하게 된다. TASK-381에는 그 문제가 없다. 변경이 오늘 일어났고
맥락이 살아 있으므로 **레거시 선언이 아니라 진짜 리뷰가 가능하고, 더 싸다.** 독립
리뷰어(`review-381`, 작업 미참여)가 판정하고 영수증을 `tasks/receipts/` 아래 남긴다.

이 절은 배분만 소유한다는 원칙은 그대로다. 수는 여전히 `ce task lint`가 센다.

**결과(같은 날 완료).** `review-381`이 판정했다 — **`conditional`**. 게이트가 공허하지
않다는 것은 섰다: 리뷰어가 결함 네 형태를 독립적으로 재파종해 전부 rc=1로 발화했다.
조건이 붙은 이유는 범용 수량사 오탐 클래스이고, 미해결 발견은 [[ISSUE-016]]·[[ISSUE-017]]·
[[ISSUE-018]]로 분리했다. 영수증은
`tasks/receipts/TASK-381/done-review-d2dfbf1c….json`이다. `ce task lint`의 미리뷰
집계가 38 → 37로 줄었다.

배분 판단이 옳았음을 한 가지가 확인해 준다: 리뷰가 **실제 결함 여섯을 찾았다.**
레거시 선언으로 닫았다면 그 여섯은 지금도 보이지 않았을 것이고, 그중 둘(PLAN-007
scope의 `결함 3건`, PLAN-009 본문의 `필드 32개`)은 **이 계획 문서 자신이 밟게 될
지뢰**였다. provenance가 살아 있는 카드에 진짜 리뷰를 붙이는 비용은 회수된다.

영수증 핀은 정규 digest가 아니라 plain sha256이다. TASK-381이 `blocks:`를 선언하지
않아 validator의 영수증 검사에 도달하지 않고, 따라서 정규 digest를 얻을 경로가 없다.
가짜 `blocks:`로 그 경로를 여는 선택지는 버렸다 — 없는 의존을 보드에 만드는 일이다.
이 비대칭 자체가 [[ISSUE-015]]가 소유한 결함이다.

### 2026-09-14 처분 — 컷오프는 날짜가 아니라 규칙이 생긴 커밋이다

위 방침("소급 리뷰하지 않는다")을 실제로 집행했다. 다만 **레거시를 임의의 날짜로
가르지 않았다.** 규칙이 없던 때에 done으로 간 카드에만 waiver가 정직하고, 규칙이
있는데도 리뷰 없이 간 카드는 레거시가 아니라 부채다. 그래서 경계를 규칙 자체에
맞췄다:

```
$ git log --format='%h %ad %s' --date=short -S'## Task completion review' -- AGENTS.md | tail -1
1c85d8d0 2026-09-10 docs(tasks): require independent done review
```

각 카드가 `done/`으로 들어온 커밋(`git log --diff-filter=A -1 -- <path>`)의 시각을
이 커밋과 비교해 둘로 갈랐다.
...
근거는 일괄이 아니다 — 어느 카드가 왜 레거시인지 카드 안에서 확인된다.

**남은 8장은 수가 줄었다는 뜻이 아니라 성격이 바뀌었다는 뜻이다.** 37장은 "리뷰
안 됨"이라는 한 덩어리였고, 지금 남은 8장은 "리뷰해야 하는데 안 한 것"이다. 앞의
것은 기록으로 닫히고 뒤의 것은 작업으로만 닫힌다. `ce task lint`의 `UNFINALIZED
DONE` 줄이 이제 그 8장만 센다.
