...
위 2026-09-10 항목의 46장/9장은 더 이상 재고가 아니다. 오늘 `tasks/done/`은 **68장**이고
...
쓰는 것은 `waived`를 `pass`로 위장하는 것과 같고, 아래 §`waived`가 상류에 추가됐다가
...
- 유입은 게이트가 막는다. `blocks:`를 선언한 done 카드는 `ce task validate`가 receipt를
  요구하고, [[TASK-383]]이 그 receipt를 둘 durable 경로를 열었으므로 그 걸림은 이제
