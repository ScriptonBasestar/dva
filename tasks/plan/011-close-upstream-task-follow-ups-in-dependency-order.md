...
dependencies, independent reviews, and explicit ownership blockers. Keep TASK-421
and TASK-436 visible while their respective product-owner and human-reclamation
...
| 완료/차단 | TASK-435 issue-promote guard, TASK-431 host tool pin | TASK-435는 상류 통합 완료; TASK-431 source는 통합됐으나 live install이 보존 대상 drift에서 멈춤 |
...
| 차단 | TASK-430 finalize evidence path | prose/reference 경로는 정리했으나 TASK-410 canonical receipt의 보존/소유 계약이 ISSUE-024에서 미결 |
...
| 차단 | TASK-424 validate zone/status ownership | `0f0a5f3c`는 live-zone unknown/empty/mismatch를 거부하지만 archive 검사, 누락 status, exact literal은 미포함(ISSUE-041). full parity를 확인하기 전까지 DVA의 기존 guard를 유지하고, readiness verdict는 `ce task gate`에 둔다 |
...
| 사람 전용 | TASK-436 duplicate TASK-407 worktree 회수 | 로컬 커밋은 저장돼 있고 branch는 source tip보다 오래됐으며 upstream도 없음; 사람의 참조/내용 결정 뒤 |
