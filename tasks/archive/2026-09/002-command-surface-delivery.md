...
| D6/D7 validate 경고 | 구현 가능 | [TASK-244](244-validate-plan-declaration-drift.md) |
| `config env` bridge | Option A 확정 | [TASK-245](245-freeze-env-bridge-contract.md) → [TASK-246](246-implement-secure-config-env-bridge.md) |
| env bridge safe write가 target 디렉토리에 고정되지 않음 | 리뷰에서 확인, 재현됨 | [TASK-284](284-anchor-env-safewrite-to-target-directory.md) |
| required env 오류 전파 | owner 복구 완료·env_file 결정 필요 | [TASK-247](247-freeze-required-env-command-policy.md) → [TASK-264](264-restore-imported-command-ownership.md) → [TASK-248](../../done/248-enforce-required-env-command-policy.md) |
...
| migration gate | **N/A 종료** — TASK-252가 승격을 기각해 소비처가 사라짐 | [TASK-251](251-build-env-migration-evidence-gate.md) |
...
[TASK-245](245-freeze-env-bridge-contract.md) §7-4/§8-5는 임시 파일을 config
root에 `.dva-env-` **접두사**로 만드는 형태로 동결했지만,
[TASK-284](284-anchor-env-safewrite-to-target-directory.md)(`bbe3db1`)가 temp를
...
**완료 (2026-09-07):** TASK-249가 `done/`으로 옮겨지며 위 16장 전부 닫혔다.
...
- 게이트 없는 `config env show`, top-level `dva env show`, lifecycle auto-unseal
  ...
  [TASK-245](245-freeze-env-bridge-contract.md) §11의 무조건 기각을 그만큼 supersede한다)
