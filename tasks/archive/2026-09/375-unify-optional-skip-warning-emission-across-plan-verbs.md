...
verification-summary: |
  ...
  TestOptionalSkipIsReportedByComposition|TestOptionalSkip|TestUsageWarningExamplesMatchTheRenderedFormat
  ok; ORDERING RULE comment at plan_lifecycle.go:272; emission sites all
  resolve-then-warn; make doc-check exit 0 (batch run).
...
### review-375 F8 (Minor, 후속 수정)

리뷰어가 `pass` 와 함께 낸 비차단 발견: `compositionVarsFixture` 가 `composes[].vars` 로
`CHECKOUT` 을 덮어쓰지만 엔트리의 dir 는 리터럴 `vendor/api` 였다 — **경고가 읽는 값이
그 var 를 참조하지 않는다.** 두 해석의 var map 은 달랐지만 관측 가능한 어떤 것도 거기에
의존하지 않아서, 비교가 구조적으로 달라질 수 없었다. 주석이 이름 붙인 바로 그 미래 변경
(`resolver.go:428` 위에서 dir 에 `resolved.EnvVars` 를 전개)을 실제로 구현해도 가드는
**통과했다.**

한 토큰 수정: `dir: vendor/api` → `dir: vendor/${CHECKOUT}`. 직접 측정:

| | 리터럴 dir (수정 전) | `${CHECKOUT}` dir (수정 후) |
|---|---|---|
| resolver 원본 | 통과 | 통과 (오늘은 아무도 dir 를 전개하지 않으므로 양쪽이 `${CHECKOUT}` 그대로를 본다) |
| resolver 에 dir 전개 주입 | **통과** — 가드가 무력 | **FAIL** — `vendor/somewhere-else` vs `vendor/vendor` 로 차이를 그대로 출력 |

단언 순서도 바꿨다. `slices.Equal` 비교를 먼저 하고 fixture 건전성 검사를 뒤로 보낸다 —
그러지 않으면 차이가 실재할 때 건전성 검사가 먼저 터져 정작 유용한 메시지(어느 경로가
어느 디렉토리를 봤는지)를 가린다.

**빈 경고 `t.Fatal` 은 유지했고, 검사 하나를 더 붙였다.** 둘은 다른 공허함을 막는다 —
전자는 "테스트가 아무것도 관측하지 못했다", 후자는 "관측은 했는데 그 관측이 차이에
반응할 수 없는 값이다". 후자가 덜 눈에 띄고, 이번에 실제로 놓친 쪽이다.

**배운 것**: 미래의 분기를 막는 회귀 테스트는 단언이 틀리는 실패와 **fixture 가 무력한**
