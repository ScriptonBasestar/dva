...
verification-summary: |
  tools/dogfoodrun/dogfood-run.sh no-arg rc 0 plan-only output (no up/down/build
  executed); --preview task348 rc 0 read-only purge scope; CONTROL_COMMIT=275c8c98
  pinned with rationale in card; fixtures present; make doc-check exit 0 (batch run).
  The --execute path remains human-only per the card's safety contract (not run).
