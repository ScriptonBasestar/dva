...
verification-summary: |
  tools/familybook-readiness-landed.sh runs clean; live fetch now reports LANDED rc 0
  (origin/develop 114e69cf2f12 accepts both dva.yml and dva.yaml, --offline agrees) —
  the watch fired as designed and TASK-329 is unblocked, which supersedes the card's
  2026-09-13 NOT LANDED observation. TASK-329 links TASK-378 (grep rc 0); fetch goes
  to a temp dir, familybook repo untouched. make doc-check exit 0 (batch run).
...
quality-review-receipt: tasks/done/evidence/TASK-378/done-review-9d7f1dc95a3117f3599eaed9ef7fc5fe68761496fe6b7c1178c619a0b686f8fa.json
---
...
- [x] 착지 조건이 exit code로 답하는 검사로 고정돼 있다 | verify: human — 검사를 지금 실행하면 "아직 착지하지 않음"으로 비영 종료하는 것이 확인된다
- [x] 검사가 familybook 저장소에 아무것도 쓰지 않는다 | verify: human — 검사 실행 전후로 그 저장소의 `git status`가 동일함이 확인된다
- [x] TASK-329가 이 검사를 링크한다 | verify: `/usr/bin/grep -rq --include='329-rename-familybook-devbox-dva-yaml-to-dva-yml-after-readiness-branch-lands.md' 'TASK-378' tasks` (regression-guard)
