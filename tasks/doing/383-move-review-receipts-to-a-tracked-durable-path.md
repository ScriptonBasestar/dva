...
**근거.** 설치된 `ce` 0.8.4 바이너리가 내는 receipt 관련 메시지는 셋뿐이고 셋 다 경로에
대해 아무것도 요구하지 않는다.
...
```
Summary: 82 valid, 2 invalid (total: 84)
```
...
- [ ] live done 카드 6장의 `quality-review-receipt`가 전부 Git 추적 경로를 가리킨다 | verify: `! /usr/bin/grep -rn '^quality-review-receipt: tmp/' tasks/done/`
- [ ] 옮겨진 receipt 6건이 원본과 바이트 동일하다 | verify: `human — cmp로 6건 확인, 이 카드 §실측에 기록`
- [ ] `tmp/`가 없는 체크아웃에서 validate 실패가 6에서 2로 줄고, 남은 2건은 TASK-344·371이다 | verify: `ce task validate --all`
- [ ] 문서 게이트가 새 디렉토리를 받아들인다 | verify: `make doc-check` (regression-guard)
- [ ] 경로 관례가 AGENTS.md와 `tasks/receipts/README.md`에 적혀 있다 | verify: `/usr/bin/grep -q 'tasks/receipts' AGENTS.md`
- [ ] ISSUE-001이 3번의 소유권 정정을 반영한다 | verify: `/usr/bin/grep -rq --include='001-task-runtime-cannot-review-legacy-done-cards-without-verification-evidence.md' 'TASK-383' tasks`
