...
```bash
set -euo pipefail
go install github.com/ScriptonBasestar/dva/cmd/dva@v0.3.0
go_bin="$(go env GOBIN)"
...
Go toolchain 없이 설치하려면 [v0.3.0 Release](https://github.com/ScriptonBasestar/dva/releases/tag/v0.3.0)에서
...
**관용 범위**: `optional: true`가 봐주는 것은 **체크아웃되지 않은 디렉토리**뿐입니다.
선언 자체가 깨진 엔트리 — 존재하지 않는 러너 이름, 해석되지 않는 러너 — 는 `optional`을
달아도 조용히 빠지지 않고 **plan 전체를 실패시킵니다**. 없는 디렉토리는 이 필드가 존재하는
이유지만, 해석되지 않는 선언은 오타이고, 오타를 조용히 떨어뜨리면 영원히 드러나지 않습니다.
