...
3. **Plan scope** — Derive self-contained plans from the capability closure in
   ...
   belongs in `plans.*.entries[].services`; Compose profile selection belongs in
   `plans.*.entries[].profiles`; plans do not inherit or overlay each other.
...
12. **`env_file:` object format** — Prefer `{ files: [...] }` (optional `required:`); plain string path is also valid. When the project shows sops evidence (`.sops.yaml`, `*.enc`, `*.sops.*`, `secrets.sources.*.sops`), the entry that file decrypts to must declare `sops_source:`; without evidence, never invent one.
...
14. **Section order (canonical)** — Canonical source:
    ...
    `version` → `vars` → `environment` → `env_file` → `stack` → `plans` → `default_plan` → `environments` → `sites` → `checks` → `default_mode` → `suggestion_ignore` → `suggestions` → `drift_ignore` → `modes` → `health_checks` → `interaction` → `provision` → `modules` → `subprojects` → `endpoints` → `infra` → `ssh` → `devcontainer`
