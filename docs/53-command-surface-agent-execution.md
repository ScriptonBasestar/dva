...
이 문서는 [PLAN-002](../tasks/_archive/2026-09/002-command-surface-delivery.md)를 새 Codex 세션에서
실행 세션 규칙과 시작 프롬프트를 소유한다.
제품·보안 계약과 acceptance criteria는 PLAN-002와 각 task card가 정본이며 여기서 반복하지
...
PLAN-002는 public CLI, secret safety, filesystem atomicity와 cross-repository promotion을
...
각 항목은 별도 writable session이며 source branch 통합은 직렬 수행한다.

| Wave | Task | 세션 종료 조건 |
| --- | --- | --- |
| 완료 | [TASK-245](../tasks/_archive/2026-09/245-freeze-env-bridge-contract.md) | Option A 결정 승인 완료 |
| 완료 | [TASK-247](../tasks/_archive/2026-09/247-freeze-required-env-command-policy.md) | 결정·검증·통합 완료 |
| 1 | [TASK-249](../tasks/_archive/2026-09/249-redesign-capability-driven-init.md) | init contract 사용자 결정이 기록·검증됨 |
| 1 | [TASK-244](../tasks/_archive/2026-09/244-validate-plan-declaration-drift.md) | D6/D7 구현·독립 review·통합 완료 |
| 완료 | [TASK-264](../tasks/_archive/2026-09/264-restore-imported-command-ownership.md) | owner 복구·통합 완료 |
| 완료 | [TASK-265](../tasks/_archive/2026-09/265-decide-interaction-env-file-contract.md) | env_file 결정 승인 완료 |
| 1 | [TASK-266](../tasks/_archive/2026-09/266-deprecate-and-reject-interaction-env-file.md) | TASK-265 결정 구현 |
| 2 | [TASK-248](../tasks/_archive/2026-09/248-enforce-required-env-command-policy.md) | TASK-247·264·265 위에서 loader safety 구현·통합 완료 |
| 2 | [TASK-250](../tasks/_archive/2026-09/250-implement-capability-driven-init.md) | TASK-244·249 결과 구현·통합 완료 |
| 3 | [TASK-246](../tasks/_archive/2026-09/246-implement-secure-config-env-bridge.md) | TASK-245 결정·TASK-248 contract 위에서 구현·통합 완료 |
| 4 | [TASK-252](../tasks/_archive/2026-09/252-decide-top-level-env-promotion.md) | 영구 유지 결정 또는 promotion evidence 요청이 기록됨 |
| 조건부 | [TASK-251](../tasks/_archive/2026-09/251-build-env-migration-evidence-gate.md) | TASK-252가 요청한 경우 gate 통합, 아니면 N/A 종료 |
| 조건부 재개 | [TASK-252](../tasks/_archive/2026-09/252-decide-top-level-env-promotion.md) | TASK-251 evidence를 사용한 최종 결정 기록 |

TASK-245·265 승인됨. 다음은 TASK-248·266이다.
...
5. 별도 reviewer와 root가 실제 diff·relevant gate를 확인한다.
...
세부 계약은 [PLAN-002 §4](../tasks/_archive/2026-09/002-command-surface-delivery.md#4-env-bridge의-비협상-수용-조건),
[§5](../tasks/_archive/2026-09/002-command-surface-delivery.md#5-promotion-evidence-contract)와 target task card가 소유한다.
Root는 변경 전에 해당 조항을 확인하고, 완화·선행·우회가 필요해지면 scope를 넓히지 말고 멈춘다.

## 7. 검증과 Git 완료 조건

Build, test, service, log 작업 전에는 `dva` skill을 읽고 `dva manifest -f json`으로 실행 표면을 찾는다. DVA에 동등한 workflow가 없을 때만 raw tool을 사용한다.

Task acceptance binding과 변경에 해당하는 repository gate가 모두 exit 0이어야 한다. Reviewer는 구현자와 다른 agent여야 하며 actual diff를 읽는다. Blocker/major를 수정한 뒤 focused re-review가 PASS가 아니면
통합하지 않는다.
...
PLAN: tasks/_archive/2026-09/002-command-surface-delivery.md
...
Prompt 반복 규칙보다 repository policy와 target task의 구체 criterion이 우선한다. 모델 이름이나
