...
- **`replace` 훅의 의미.** `plans:` 필터는 세 phase(`before`/`replace`/`after`)에 똑같이
  ...
  "이 plan에서는 기본 동작을 쓴다"가 정확히 그 선언의 뜻이기 때문이다. 되돌아갈 내장
  동작이 없는 `mode.build: native`의 중첩 경로만 예외로, 거기서는 에러로 실패한다
  (`internal/cli/compose.go`). 그 자리에서 조용히 넘어가면 빌드가 없었다는 사실이
  아무 데도 남지 않는다.
